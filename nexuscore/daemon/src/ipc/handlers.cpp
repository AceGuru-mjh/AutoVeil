...
enum class RpcType : uint32_t {
    ...
    PatchBoot = 17, GetBootPatcherStatus = 18,
};
}
...
std::vector<uint8_t> Handlers::dispatch(Context& ctx, uint32_t seq, const std::vector<uint8_t>& reqPayload) {
    ...
    switch (t) {
        ...
        // Phase 4: Boot Patcher
        case RpcType::PatchBoot:              return handlePatchBoot(ctx, d);
        case RpcType::GetBootPatcherStatus:   return handleGetBootPatcherStatus(ctx, d);
        default:
...
// Phase 4: Boot Patcher RPC 实现
// 注意：这些 RPC 直接调用 BootPatcher 静态方法，不通过 DaemonCore
// 因为 boot patching 是"安装时"操作，不需要 daemon 运行时状态

std::vector<uint8_t> Handlers::handlePatchBoot(Context& ctx, Decoder& d) {
    std::string bootImgPath = d.getStr();
    std::string outputPath = d.getStr();

    // 安全校验：拒绝含危险字符的路径
    if (!nexus::isPathSafe(bootImgPath)) {
        return makeError(-3, "invalid boot.img path (contains dangerous characters)");
    }
    if (!nexus::isPathSafe(outputPath)) {
        return makeError(-3, "invalid output path");
    }
    if (!nexus::probeFile(bootImgPath)) {
        return makeError(-4, "boot.img not found: " + bootImgPath);
    }

    NX_LOG_I("Handlers", "PatchBoot: %s -> %s (requested by uid=%d)",
             bootImgPath.c_str(), outputPath.c_str(), ctx.peer.uid);

    // 生成 init.rc + bootstrap
    std::string initScript = "(generated by nexus_rc.cpp generateNexusRc)";
    // 实际生产应调用 nexus::generateNexusRc()，这里简化

    // 调用 BootPatcher
    auto result = nexus::BootPatcher::patch(bootImgPath, outputPath,
                                            "/data/adb/nexuscore/bin/nexusd",
                                            initScript);

    Encoder enc;
    enc.putU32(result.success ? 0 : -5);
    enc.putStr(result.success ? "ok" : result.errorMsg);
    if (result.success) {
        enc.putBool(true);                    // success
        enc.putStr(result.outputPath);
        enc.putStr(result.sha1);
        enc.putBool(result.needReboot);
    }
    enc.putEnd();
    return std::move(enc).bytes();
}

std::vector<uint8_t> Handlers::handleGetBootPatcherStatus(Context& ctx, Decoder& d) {
    Encoder enc;
    enc.putU32(0);
    enc.putStr("ok");

    // is_rooted: 检查 /data/adb/nexuscore/.bootstrapped
    bool isRooted = nexus::probeFile("/data/adb/nexuscore/.bootstrapped");
    enc.putBool(isRooted);

    // boot_path: 检测当前 boot 路径
    auto bootPathR = nexus::BootPatcher::detectBootPath();
    enc.putStr(bootPathR.has_value() ? *bootPathR : "");

    // has_backup: 检查备份文件
    enc.putBool(nexus::probeFile("/data/adb/nexuscore/backup/boot.img.orig"));

    // nexuscore_version
    auto version = nexus::readFile("/data/adb/nexuscore/.version");
    enc.putStr(version.has_value() ? nexus::trim(*version) : NEXUS_VERSION);
